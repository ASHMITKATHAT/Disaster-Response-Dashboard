...
class DataIntegration:
    ...
    async def fetch_nasa_gpm_data(self, lat: float, lon: float, 
        ...
        try:
            ...
            # Make request (simplified - actual NASA API is more complex)
            async with self.session.get(
                "https://gpm1.gesdisc.eosdis.nasa.gov/opendap/hyrax/GPM_L3/GPM_3IMERGHH.06/data",
                params=params
            ) as response:
                if response.status == 200:
                    ...
                    rainfall_data = self._process_nasa_rainfall(data)
                    
                    result = {
                        'latitude': lat,
                        'longitude': lon,
                        'rainfall_mm': rainfall_data.get('current', 0),
                        'hourly_rainfall': rainfall_data.get('hourly', []),
                        'cumulative_6h': rainfall_data.get('cumulative_6h', 0),
                    ...
                    logger.info(f"NASA GPM data fetched: {rainfall_data.get('current', 0)}mm")
                    return result
                else:
    ...
    async def fetch_isro_lulc_data(self, lat: float, lon: float,
        ...
        try:
            # ISRO Bhuvan WMS parameters
            params = {
                'service': 'WMS',
                'version': '1.1.1',
                'request': 'GetFeatureInfo',
                'layers': 'BHUVAN:LULC_50K',
                'query_layers': 'BHUVAN:LULC_50K',
            ...
            headers = {
                'Authorization': f'Bearer {config.ISRO_LULC_API_KEY}'
            }
            
            async with self.session.get(
                config.ISRO_BHUVAN_WMS,
                params=params,
                headers=headers
            ) as response:
                if response.status == 200:
                    ...
                    # Process LULC data
                    lulc_data = self._process_lulc_data(data)
                    
                    result = {
                        'latitude': lat,
                        'longitude': lon,
                        'land_use_class': lulc_data.get('primary_class', 'Unknown'),
                        'land_use_code': lulc_data.get('class_code', 0),
                        'confidence': lulc_data.get('confidence', 0),
                        'area_analysis': lulc_data.get('area_stats', {}),
                        'timestamp': datetime.now().isoformat(),
                        'data_source': 'ISRO_Bhuvan_LULC'
                    }
                    ...
                    logger.error(f"ISRO API error: {response.status}")
